...
public class TimedCenterBackDoubleScoreAuton extends SequentialCommandGroup {
  ...
  public TimedCenterBackDoubleScoreAuton(DriveTrain dt, Dumper d) { 
    ...
    addCommands(new AutonDumperMoveLimitSwitch(d),
        new ParallelCommandGroup(new DriveTimed(dt, Constants.autonSpeed, 1.5f), new DumperIntake(d).raceWith(new Wait(1.75f))), 
        new ParallelCommandGroup(new DumperMoveLimitSwitch(d), new TurnTimed(dt, -Constants.autonTSpeed, 2.1f)), 
        new DriveTimed(dt, Constants.autonSpeed, 2.0f), 
        new TurnTimed(dt, -Constants.autonTSpeed, 0.35f), 
        new DriveTimed(dt, Constants.autonSpeed, 1.0f),
        new ParallelCommandGroup(new DriveTimed(dt, Constants.autonSpeed, 0.5f), new DumperVomit(d).raceWith(new Wait(2.0f))),
